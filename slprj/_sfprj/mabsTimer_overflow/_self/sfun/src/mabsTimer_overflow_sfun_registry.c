...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_mabsTimer_overflow_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_mabsTimer_overflow_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_mabsTimer_overflow_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_mabsTimer_overflow_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_mabsTimer_overflow_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "mabsTimer_overflow") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_mabsTimer_overflow_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_mabsTimer_overflow_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"mabsTimer_overflow")) {
    return(sf_mabsTimer_overflow_method_dispatcher(simstructPtr,chartFileNumber,
            method,data));
  }
...
extern void mabsTimer_overflow_terminator(void);
...
extern void mabsTimer_overflow_initializer(void);
extern void mabsTimer_overflow_register_exported_symbols(SimStruct* S);
extern void mabsTimer_overflow_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  mabsTimer_overflow_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      mabsTimer_overflow_debug_initialize();
    }

    mabsTimer_overflow_initializer();
  }
